...
function SignInForm() {
  ...
  React.useEffect(() => {
    if (errorParam) {
      showToast('Authentication Error', 'error', errorParam);
    }
  ...
  const onSubmit = async (data: SignInInput) => {
    ...
    try {
      ...
      if (res.success && res.user) {
        ...
        // Retrieve role from database response
        const role = (res as any).role || 'customer';
        const targetPath = redirectTo || (role === 'admin' ? '/admin/dashboard' : '/customer/dashboard');
        
        router.push(targetPath);
        router.refresh();
      } else {
        showToast('Sign In Failed', 'error', res.error || 'Invalid credentials.');
      }
    } catch (err: any) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      showToast('Google Sign In Failed', 'error', err.message);
      ...
      <div className="w-full max-w-md space-y-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Sign in to manage your account or start shopping
          </p>
        </div>
  ...
  return (
    <React.Suspense fallback={
      <div className="flex min-h-screen items-center justify-center bg-background px-4 py-12">
        <div className="text-center text-sm text-muted-foreground">Loading sign in...</div>
      </div>
    }>
      <SignInForm />
    </React.Suspense>
