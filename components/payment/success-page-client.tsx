...
export function SuccessPageClient({ orderNumber }: SuccessPageClientProps) {
  ...
  if (orderStatus === 'pending') {
    return (
      <div className="mx-auto w-full max-w-md px-4 py-24 text-center space-y-6 flex-1 flex flex-col justify-center items-center">
        <div className="p-4 bg-indigo-50 dark:bg-indigo-950/20 text-indigo-600 rounded-full border border-indigo-100 dark:border-indigo-900/40 relative">
        ...
        {orderNumber && (
          <div className="p-4 bg-slate-50 dark:bg-slate-900/40 border border-slate-200/60 dark:border-slate-800 rounded-xl w-full text-xs font-semibold flex items-center justify-between">
            <span className="text-slate-400">Order Reference:</span>
            <span className="font-mono text-slate-800 dark:text-slate-200">{orderNumber}</span>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="mx-auto w-full max-w-md px-4 py-16 text-center space-y-6 flex-1 flex flex-col justify-center items-center animate-fade-in">
      ...
      {orderNumber && (
        <div className="p-3.5 bg-slate-50 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800 rounded-xl w-full text-xs font-semibold flex items-center justify-between">
          <span className="text-slate-400">Order Reference:</span>
          <span className="font-mono text-slate-800 dark:text-slate-200">{orderNumber}</span>
        </div>
      )}
